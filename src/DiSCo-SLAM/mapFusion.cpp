...
//        return;
    ...
    bool getInitialGuess(ScanContextBin bin, int idx_nearest, int min_idx){
        ...
        PointTypePose pose_source_lidar = ndtRelativeMotion(transformPointCloud(bin.cloud, &source_pose_initial),
                                                            transformPointCloud(bin_nearest.cloud, &target_pose), source_pose_initial);
...
//        }
    ...
    PointTypePose ndtRelativeMotion(pcl::PointCloud<PointType>::Ptr source,
                                pcl::PointCloud<PointType>::Ptr target,
                                PointTypePose pose_source)
    {
        // NDT Settings
        ...
        ndt.setStepSize(0.25);  // Set an appropriate step size
        ...
        *target = *cloud_temp;
        ...
        correctionLidarFrame = ndt.getFinalTransformation();  // get transformation in camera frame
        ...
        pose_from.roll = roll;
        pose_from.pitch = pitch;
        pose_from.intensity = ndt.getFitnessScore();
